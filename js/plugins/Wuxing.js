...
var phrases = [
	"Bois puise Terre",
	"Bois nourrit Feu",
	"Feu fait fondre Metal",
	"Feu crée la Terre",
	"Terre absorbe Eau",
	"Terre contient Metal",
	"Metal tranche Bois",
	"Metal collecte Eau",
	"Eau éteint Feu",
	"Eau nourrit Bois",
...
var elementSFX = {
	bois: params["Wood Win SFX"].split(", "),
	feu: params["Fire Win SFX"].split(", "),
	terre: params["Earth Win SFX"].split(", "),
	metal: params["Metal Win SFX"].split(", "),
	eau: params["Water Win SFX"].split(", "),
...
Scene_WuxingMG.prototype.update = function() {
	...
	if (this._shouldProcessEnd) {
		...
		if (p <= c || $gameSwitches.value(224) == true){
			this._replayCmd.open();
			this._replayCmd.activate();
		}else{
			$gameSwitches.setValue(224, true);
			this.popScene();
		};
