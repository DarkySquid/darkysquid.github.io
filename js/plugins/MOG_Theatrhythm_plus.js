...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_Theatrhythm');  
	...
	var slow_time = 60*5;
	var slow_used = false;
	var boss1stPhaseDone = false;
	var boss2ndPhaseDone = false;
	var infinite_hp = false;
	var dragospin_frame = 0;
	var bossDone = false;
	var pouletBraised = false;
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if (command === "theatrhythm")  {
		 if (args.length >= 4) {		
			 ...
			 default_speed = speed;
			 $gameVariables.setValue(speed_var, speed)
			 $gameVariables.setValue(time_var, 0)
...
Scene_Theatrhythm.prototype.initialize = function() {
	...
	boss1stPhaseDone = false;
	boss2ndPhaseDone = false;
	infinite_hp = false;
	dragospin_frame = 0;
	bossDone = false;
...
Scene_Theatrhythm.prototype.createEnemyName = function() {	
    this._enemyName = new Sprite(new Bitmap(500,32))
	...
	this._enemyName.bitmap.drawText(this._enemySprite._enemy.name(),0,0,500,32,"left")
...
Scene_Theatrhythm.prototype.update_enemyHpNumber = function() {	
	...
	if (infinite_hp == true){this.refresh_number(this._enemyHpNumber,999999999 ,this._enemyHpNumber_data,this._enemyHpNumber_data[2]);};
};
...
Scene_Theatrhythm.prototype.refresh_chain = function(sprites,value,img_data,x) {
	...
	if ((value == 20 && $gameVariables.value(difficulty_var) == 2) || (value == 20)){
		inv_used = false; //CAPACITES RECHARGE
		inv_time = 60*3;
		slow_used = false;
		slow_time = 60*5;
	};
...
Scene_Theatrhythm.prototype.update_commands = function() {
	$gameVariables.setValue(time_var, $gameVariables.value(time_var) + 1);
	if (Input.isTriggered("a")) {this.check_key(0)}
	else if (Input.isTriggered("up")) {this.check_key(1)}
	else if (Input.isTriggered("e")) {this.check_key(2)}
	else if (Input.isTriggered("i")) {this.check_key(3)}
	else if (Input.isTriggered("o")) {this.check_key(4)}
	else if (Input.isTriggered("p")) {this.check_key(5)}
	else if (Input.isTriggered("r")) {this.check_key(6)}
	else if (Input.isTriggered("t")) {this.check_key(7)}
	else if (Input.isTriggered("y")) {this.check_key(8)}
	else if (Input.isTriggered("u")) {this.check_key(9)}
	else if (Input.isTriggered("left")) {this.check_key(10)}
	else if (Input.isTriggered("down")) {this.check_key(11)}
	else if (Input.isTriggered("right")) {this.check_key(12)}
	else if (Input.isTriggered("f")) {this.check_key(13)}
	else if (Input.isTriggered("g")) {this.check_key(14)}
	else if (Input.isTriggered("h")) {this.check_key(15)}
	else if (Input.isTriggered("j")) {this.check_key(16)}
	else if (Input.isTriggered("k")) {this.check_key(17)}
	else if (Input.isTriggered("l")) {this.check_key(18)}
	else if (Input.isTriggered("m")) {this.check_key(19)}
	else if (Input.isTriggered("w")) {this.check_key(20)}
	else if (Input.isTriggered("x")) {this.check_key(21)}
	else if (Input.isTriggered("c")) {this.check_key(22)}
	else if (Input.isTriggered("v")) {this.check_key(23)}
	else if (Input.isTriggered("b")) {this.check_key(24)}
	else if (Input.isTriggered("n")) {this.check_key(25)}
	else if (Input.isTriggered("ok")) {this.slowBattle()};
};
...
Scene_Theatrhythm.prototype.execute_damage = function(target) {
    ...
    if (target === 0) {
		...
		if($gameVariables.value(song_id_var) == 9){
			dmg = 999999;
		};
		if(inv_used == false || inv_time <= 0){ //MASTER BALLS SPECIAL
...
Scene_Theatrhythm.prototype.set_total = function() {
	var n = this.score()/2 + this._enemy_bp*5 + this.chainBonus()/2;
	if ($gameParty.members()[0].hasArmor($dataArmors[4])){ //POINTS SPECIAL
		n = n*1.5
	};
	return Math.floor(n);
};
...
Scene_Theatrhythm.prototype.draw_result = function() {
	this._text_sprite.bitmap.drawText(String(this.score()) + " * 0.5",0,0,200,32,"right");
	this._text_sprite.bitmap.drawText("(" + String(this._result_data[0]) + ") " + String(this.chainBonus()) + " * 0.5",0,32,200,32,"right");
	this._text_sprite.bitmap.drawText(String(this._enemy_bp) + " * 5",0,64,200,32,"right"); 
...
Scene_Theatrhythm.prototype.update = function() {
	...
	if ($gameSystem._theatrhythm_miss_dmg) {
		if ($gameParty.members()[0].hasArmor($dataArmors[14]) && inv_used == false) {
			inv_used = true;
			this._battlers[0].startAnimation(51, false, 0); //MASTER BALLS SPECIAL
		}
		if (inv_used == true && inv_time > 0){
			inv_time -= 1;
			if (inv_time == 20){
				this._battlers[0].startAnimation(54, false, 0);
			}
		}else{
			this.execute_miss_damage();
		}
	};
	if (slow_used == true && slow_time > 0){
		slow_time -= 1;
		if (slow_time == 20){ //SLOW SPECIAL
			this._battlers[0].startAnimation(54, false, 0);
		}
		if(slow_time == 1){
			default_speed += 2;
...
Sprite_TKeys.prototype.initialize = function(img,pos) {
	...
	this._image = img;
	this._cw = this._image.width  / keys_number;
...
Sprite_TKeys.prototype.refresh_keys = function(i) {
	...
	this._keys_f[i] = false;
	var difficulty = $gameVariables.value(difficulty_var) || 0;
	if (difficulty == 1){
		this._keys_c[i] = Math.randomInt(keys_number - 16);
	}else{
		this._keys_c[i] = Math.randomInt(keys_number - 10*difficulty);
	}
...
Sprite_TKeys.prototype.update_position = function(i) {
	this._keys_speed = $gameVariables.value(speed_var);
	this._keys[i].x += this._keys_speed;
	if (boss2ndPhaseDone == true){
		this._keys[i].rotation += 0.2;
	};
...
// * slowBattle
//===============================
Scene_Theatrhythm.prototype.slowBattle = function() {
	if ($gameParty.members()[0].hasArmor($dataArmors[9]) && slow_used == false) {
		this._battlers[0].startAnimation(51, false, 0);
		slow_used = true; //SLOW SPECIAL
		slow_time = 60*5;
		default_speed -= 2;
	};
};
...
// * changeSpeed
//===============================
Scene_Theatrhythm.prototype.changeSpeed = function() {
	$gameVariables.setValue(speed_var, default_speed);
	switch ($gameVariables.value(song_id_var)) {
		case 0: //Dio - Rainbow in the Dark
			this._enemySprite._enemy._theatrhythm_ani_id = 3;
			if ($gameVariables.value(time_var) >= 60*265) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
			break;
		case 1: //Iron Maiden - Transylvania
			this._enemySprite._enemy._theatrhythm_ani_id = 1;
            if ($gameVariables.value(time_var) >= 60*20 + 150 - 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*96 + 150 - 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*105 + 150 - 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*114 + 150 - 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*150 + 150 - 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*168 + 150 - 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*204 + 150 - 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*219 + 150 - 60) {
				$gameVariables.setValue(speed_var, default_speed - 1);
			};
			if ($gameVariables.value(time_var) >= 60*235 + 150 - 60) {
				$gameVariables.setValue(speed_var, default_speed - 2);
			};
			if ($gameVariables.value(time_var) >= 60*243 + 150 - 60) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
            break;
		case 2: //Black Sabbath - Heaven and Hell
			this._enemySprite._enemy._theatrhythm_ani_id = 16;
			if ($gameVariables.value(time_var) >= 60*0) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*22 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*55 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*60 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*71 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*98 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*105 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*136 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*158 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*105 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*193 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*243 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*256 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*275 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 4);
			};
			if ($gameVariables.value(time_var) >= 60*350 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*400 + 60) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
			break;
		case 3: //Rush - YYZ
			this._enemySprite._enemy._theatrhythm_ani_id = 67;
			if ($gameVariables.value(time_var) >= 60*5 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*32 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*36 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*40 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*43 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*171 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*199 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*252 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*262 + 120/2) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
			break;
		case 4: //Muse - Hysteria
			this._enemySprite._enemy._theatrhythm_ani_id = 2;
			if ($gameVariables.value(time_var) >= 60*20 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*42 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*62 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*82 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*93 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*134 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*145 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*166 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*207 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*223 + 120/2) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
			break;
		case 5: //Epica - Abyss Of Time
			this._enemySprite._enemy._theatrhythm_ani_id = 77;
			if ($gameVariables.value(time_var) >= 60*13 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed - 2);
			};
			if ($gameVariables.value(time_var) >= 60*16 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*44 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed - 1);
			};
			if ($gameVariables.value(time_var) >= 60*74 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*116 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed - 1);
			};
			if ($gameVariables.value(time_var) >= 60*188 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*220 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed - 2);
			};
			if ($gameVariables.value(time_var) >= 60*238 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed - 1);
			};
			if ($gameVariables.value(time_var) >= 60*279 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*293 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*308 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed - 2);
			};
			if ($gameVariables.value(time_var) >= 60*314 + 120/2) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
			break;
		case 6: //Rainbow - Gates of Babylon
			this._enemySprite._enemy._theatrhythm_ani_id = 106;
			if ($gameVariables.value(time_var) >= 60*54 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*109 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*116 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*167 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*175 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*227 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*257 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*284 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*304 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*352 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*358 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*382 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*407 + 120/2) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
			break;
		case 7: //Stratovarius - Black Diamond
			this._enemySprite._enemy._theatrhythm_ani_id = 5;
			if ($gameVariables.value(time_var) >= 60*12 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*25 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*51 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*102 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*128 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*139 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*153 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*204 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*282 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*300 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*334 + 120/2) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
			break;
		case 8: //Ozric Tentacles - Myriapod
			this._enemySprite._enemy._theatrhythm_ani_id = 81;
			if ($gameVariables.value(time_var) >= 60*14 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*38 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*65 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*89 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*113 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*149 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*173 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*207 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			...
			if ($gameVariables.value(time_var) >= 60*267 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*290 + 120/2) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*353 + 120/2) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
			break;
		case 9: //Nightwish - Moondance
			this._enemySprite._enemy._theatrhythm_ani_id = 55;
			if ($gameVariables.value(time_var) >= 60*18 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*44 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*110 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*149 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*182 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 4);
			};
			if ($gameVariables.value(time_var) >= 60*208 + 60) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
			break;
		case 10: //Deep Purple - Burn
			this._enemySprite._enemy._theatrhythm_ani_id = 67;
			if ($gameVariables.value(time_var) >= 60*11 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*54 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*60 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*97 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*103 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*137 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*156 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*163 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*168 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*195 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*205 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*250 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*290 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*295 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*350 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*359 + 60) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
			break;
		case 11: //Van Halen - Eruption
			this._enemySprite._enemy._theatrhythm_ani_id = 67;
			if ($gameVariables.value(time_var) >= 60*2 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*5 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*12 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*20 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*38 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*47 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*57 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 4);
			};
			if ($gameVariables.value(time_var) >= 60*73 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 5);
			};
			if ($gameVariables.value(time_var) >= 60*84 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*100 + 60) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
			break;
		case 12: //Genesis - Duke's Travels
			this._enemySprite._enemy._theatrhythm_ani_id = 71;
			if ($gameVariables.value(time_var) >= 60*13 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*58 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*116 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*134 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 4);
			};
			if ($gameVariables.value(time_var) >= 60*141 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*167 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 5);
			};
			if ($gameVariables.value(time_var) >= 60*177 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*245 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 5);
			};
			if ($gameVariables.value(time_var) >= 60*258 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*265 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 5);
			};
			if ($gameVariables.value(time_var) >= 60*277 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*283 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 4);
			};
			if ($gameVariables.value(time_var) >= 60*319 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 5);
			};
			if ($gameVariables.value(time_var) >= 60*326 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*369 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 4);
			};
			if ($gameVariables.value(time_var) >= 60*407 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*448 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*472 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*505 + 60) {
				$gameVariables.setValue(speed_var, default_speed - 1);
			};
			if ($gameVariables.value(time_var) >= 60*518 + 80) {
				$gameVariables.setValue(speed_var, default_speed + 5);
			};
			if ($gameVariables.value(time_var) >= 60*546 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*563 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*581 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*628 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*631 + 60) {
				$gameVariables.setValue(speed_var, default_speed - 1);
			};
			if ($gameVariables.value(time_var) >= 60*640 + 60) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
			break;
		case 13: //Dragonforce - Through the Fire and Flames
			this._enemySprite._enemy._theatrhythm_ani_id = 101;
			if ($gameVariables.value(time_var) >= 60*10 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*20 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*30 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*94 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*114 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*116 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*180 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*200 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*201 + 80) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*211 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 4);
			};
			if ($gameVariables.value(time_var) >= 60*231 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*240 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 4);
			};
			if ($gameVariables.value(time_var) >= 60*259 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 5);
			};
			if ($gameVariables.value(time_var) >= 60*267 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*285 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*292 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*294 + 80) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*300 + 60) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
			break;
		case 14: //Iron Maiden - Judas Be My Guide
			this._enemySprite._enemy._theatrhythm_ani_id = 102;
			if ($gameVariables.value(time_var) >= 60*15 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*40 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*52 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*88 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*111 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 4);
			};
			if ($gameVariables.value(time_var) >= 60*123 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*134 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*151 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 5);
			};
			if ($gameVariables.value(time_var) >= 60*157 + 80) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*180 + 80) {
				$gameVariables.setValue(speed_var, default_speed + 4);
			};
			if ($gameVariables.value(time_var) >= 60*184 + 60) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
			break;
		case 15: //Metallica - For Whom The Bell Tolls
			this._enemySprite._enemy._theatrhythm_ani_id = 115;
			if ($gameVariables.value(time_var) >= 60*7 + 60 && this._enemyHp_Old <= 1000){
				if (boss1stPhaseDone == false && $gameVariables.value(time_var) < 300*60){
					$gameVariables.setValue(time_var, 300*60);
				};
			};
			if ($gameVariables.value(time_var) >= 60*7 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*24 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 4);
			};
			if ($gameVariables.value(time_var) >= 60*56 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 5);
			};
			if ($gameVariables.value(time_var) >= 60*110 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 4);
			...
			if ($gameVariables.value(time_var) >= 60*232 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 5);
			};
			if ($gameVariables.value(time_var) >= 60*248 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*300) {
				if (boss1stPhaseDone == false){
					this._spriteField.removeChild(this._enemySprite);
					this._enemy = new Game_Enemy(26,Moghunter.theatrhythm_enemy_x, Math.floor(Moghunter.theatrhythm_enemy_y*1.5));	
					this._enemySprite = new Sprite_Enemy(this._enemy);
					this._spriteField.addChild(this._enemySprite);
					this._enemySprite._enemy.startAnimation(102, false, 0);
					this._spriteHudBase.removeChild(this._enemyName);
					this._enemyName = new Sprite(new Bitmap(500,32))
					this._enemyName.x = Moghunter.theatrhythm_name_x;
					this._enemyName.y = Moghunter.theatrhythm_name_y;
					this._enemyName.bitmap.fontSize = Moghunter.theatrhythm_name_font_size;
					...
					this._spriteHudBase.addChild(this._enemyName);
					AudioManager.fadeOutBgm(2);
					AudioManager.playBgm({  name: "Dream Theater - Octavarium",  volume: 80,  pitch: 100,  pan: 0});
					boss1stPhaseDone = true;
				};
				if (this._enemyHp_Old < 100000 && infinite_hp == false){
					this._enemyHp_Old += 1000;
					this._enemy_hp[0] += 1000;
				}else{
					infinite_hp = true;
				}
				if (this._enemy_hp[0] >= 999999){
					infinite_hp = true;
				}
				this._enemySprite._enemy._theatrhythm_ani_id = 103;
				$gameVariables.setValue(speed_var, default_speed + 4);
			};
			if ($gameVariables.value(time_var) >= 60*309 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*329 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 1);
			};
			if ($gameVariables.value(time_var) >= 60*335 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*342 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*348 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 4);
			};
			if ($gameVariables.value(time_var) >= 60*354 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 5);
			};
			if ($gameVariables.value(time_var) >= 60*360 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 6);
			};
			if ($gameVariables.value(time_var) >= 60*367 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 7);
			};
			if ($gameVariables.value(time_var) >= 60*390 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*397 + 60) {//TRANS2
				if (boss2ndPhaseDone == false){
					this._spriteField.removeChild(this._enemySprite);
					this._enemy = new Game_Enemy(20,Moghunter.theatrhythm_enemy_x, Math.floor(Moghunter.theatrhythm_enemy_y*1.5));	
					this._enemySprite = new Sprite_Enemy(this._enemy);
					this._spriteField.addChild(this._enemySprite);
					this._enemySprite._enemy.startAnimation(102, false, 0);
					this._spriteHudBase.removeChild(this._enemyName);
					...
					this._enemyName.bitmap.drawText(this._enemySprite._enemy.name(),0,0,500,32,"left")
					this._spriteHudBase.addChild(this._enemyName);
					boss2ndPhaseDone = true;
				};
				this._enemySprite._enemy._theatrhythm_ani_id = 105;
				dragospin_frame += 1;
				if (dragospin_frame % 24 == 0 && bossDone == false){
					this._enemySprite._enemy.startAnimation(125, false, 0);
				};
			};
			if ($gameVariables.value(time_var) >= 60*401 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 2);
			};
			if ($gameVariables.value(time_var) >= 60*439 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 3);
			};
			if ($gameVariables.value(time_var) >= 60*489 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 5);
			};
			if ($gameVariables.value(time_var) >= 60*539 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 6);
			};
			if ($gameVariables.value(time_var) >= 60*581 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			};
			if ($gameVariables.value(time_var) >= 60*620 + 60) {
				bossDone = true;
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			...
			break;
		case 16: //Metallica - For Whom The Bell Tolls EZ
			this._enemySprite._enemy._theatrhythm_ani_id = 115;
			if ($gameVariables.value(time_var) >= 60*7 + 60 && this._enemyHp_Old <= 1000){
				if (boss1stPhaseDone == false && $gameVariables.value(time_var) < 300*60){
					$gameVariables.setValue(time_var, 300*60);
			...
			if ($gameVariables.value(time_var) >= 60*300) {
				if (boss1stPhaseDone == false){
					this._spriteField.removeChild(this._enemySprite);
					this._enemy = new Game_Enemy(29,Moghunter.theatrhythm_enemy_x, Math.floor(Moghunter.theatrhythm_enemy_y*1.5));	
					this._enemySprite = new Sprite_Enemy(this._enemy);
					this._spriteField.addChild(this._enemySprite);
					this._enemySprite._enemy.startAnimation(102, false, 0);
					this._spriteHudBase.removeChild(this._enemyName);
					...
					this._enemyName.bitmap.drawText(this._enemySprite._enemy.name(),0,0,500,32,"left")
					this._spriteHudBase.addChild(this._enemyName);
					this._party_hp[0] += 500;
					if (this._party_hp[0] > this._party_hp[1]){
						this._party_hp[0] = this._party_hp[1];
					}
					this.update_actorHpNumber();
					this._battlers[0].startAnimation(47, false, 0);
					AudioManager.fadeOutBgm(2);
					AudioManager.playBgm({  name: "Dream Theater - Octavarium",  volume: 80,  pitch: 100,  pan: 0});
					boss1stPhaseDone = true;
				};
				if (this._enemyHp_Old < 100000 && infinite_hp == false){
					this._enemyHp_Old += 1000;
					this._enemy_hp[0] += 1000;
					if ($gameVariables.value(time_var) % 5 == 0){
						this.refresh_number(this._enemyHpNumber,this._enemyHp_old ,this._enemyHpNumber_data,this._enemyHpNumber_data[2]);
					}
				}else{
					infinite_hp = true;
			...
			if ($gameVariables.value(time_var) >= 60*397 + 60) {//TRANS2
				if (boss2ndPhaseDone == false){
					this._spriteField.removeChild(this._enemySprite);
					this._enemy = new Game_Enemy(30,Moghunter.theatrhythm_enemy_x, Math.floor(Moghunter.theatrhythm_enemy_y*1.5));	
					this._enemySprite = new Sprite_Enemy(this._enemy);
					this._spriteField.addChild(this._enemySprite);
					this._enemySprite._enemy.startAnimation(102, false, 0);
					this._spriteHudBase.removeChild(this._enemyName);
			...
			if ($gameVariables.value(time_var) >= 60*551 + 60) {
				$gameVariables.setValue(speed_var, default_speed + 5);
			};
			if ($gameVariables.value(time_var) >= 60*581 + 60) {
				$gameVariables.setValue(speed_var, default_speed);
			...
			break;
		case 17: //Test
			this._enemySprite._enemy._theatrhythm_ani_id = 112;
			if ($gameVariables.value(time_var) >= 60*10 + 60) {
				$gameVariables.setValue(speed_var, default_speed+1);
			};
			if ($gameVariables.value(time_var) >= 60*20 + 60) {
			...
			if ($gameVariables.value(time_var) >= 60*195 + 60) {
				if (pouletBraised == false){
					this._spriteField.removeChild(this._enemySprite);
					this._enemy = new Game_Enemy(32,Moghunter.theatrhythm_enemy_x, Math.floor(Moghunter.theatrhythm_enemy_y*1.5));	
					this._enemySprite = new Sprite_Enemy(this._enemy);
					this._spriteField.addChild(this._enemySprite);
					pouletBraised = true;
				}
			};
			if ($gameVariables.value(time_var) >= 60*212) {
				$gameSystem._theatrhythm_phase = 10;
				$gameSwitches._data[Moghunter.theatrhythm_result_switch_id] = true;
				this.update_victory_phase();
			};
